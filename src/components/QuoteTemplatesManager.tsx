...
const QuoteTemplatesManager: React.FC<Props> = ({ isOpen, onClose, planRaw }) => {
  ...
  const handleNew = () => {
    setCurrent(null);
    setName(t('quotes.templates.newTemplate'));
    setHtml(t('quotes.templates.defaultBodyHtml'));
  };
    ...
    <div className="fixed inset-0 z-[70] bg-black/40 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-5xl h-full max-h-[90vh] flex flex-col">
        <div className="px-5 py-3 border-b flex flex-wrap items-center justify-between gap-3">
          <h3 className="text-lg font-semibold">{t('quotes.templates.title')}</h3>
        ...
        <div className="flex-1 overflow-hidden flex flex-col lg:flex-row">
          <div className="w-full lg:w-64 border-b lg:border-b-0 lg:border-r overflow-y-auto">
            <div className="p-3 flex items-center justify-between">
              <span className="text-sm text-gray-700">{t('quotes.templates.list')}</span>
              <button
                className="text-xs px-2 py-1 rounded-md bg-indigo-600 text-white disabled:opacity-50"
                onClick={handleNew}
                disabled={!canUse || atLimit}
                title={!canUse ? t('quotes.templates.requirePlan') : atLimit ? t('quotes.templates.proLimit') : t('quotes.templates.newTemplate')}
              >
                <Plus className="w-4 h-4 inline" /> {t('quotes.templates.new')}
              </button>
            </div>
            {!canUse && (
              <div className="mx-3 mb-3 p-2 text-[11px] text-amber-800 bg-amber-50 rounded border border-amber-200">
                {t('quotes.templates.freeHidden')}
              </div>
            )}
            {plan === 'pro' && (
              <div className="mx-3 mb-2 text-[11px] text-gray-500">{t('quotes.templates.proLimit')}</div>
            )}
            <ul>
              {items.map(t => (
                <li key={t.id}>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {!canUse ? (
              <div className="text-sm text-gray-600">{t('quotes.templates.manageUpgrade')}</div>
            ) : (
              <div className="space-y-3">
                <div className="flex flex-col gap-2 lg:flex-row lg:flex-wrap lg:items-center">
                  <input
                    ...
                    placeholder={t('quotes.templates.placeholderName')}
                  ...
                  <button onClick={handleSetDefault} disabled={!current} className="px-3 py-2 bg-amber-500 text-white rounded-md disabled:opacity-50 flex items-center gap-1">
                    {current?.isDefault ? <StarOff className="w-4 h-4" /> : <Star className="w-4 h-4" />} {t('quotes.templates.setDefault')}
                  </button>
                </div>
                <div>
                  <div className="text-xs text-gray-500 mb-1">{t('quotes.templates.availableTokens')}</div>
