      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center space-x-4">
            <div className="w-16 h-16 bg-orange-100 rounded-xl flex items-center justify-center">
              <span className="text-orange-600 font-bold text-2xl">{supplier.name.charAt(0).toUpperCase()}</span>
            </div>
            <div>
              <h2 className="text-2xl font-bold text-gray-900">{supplier.name}</h2>
              <p className="text-sm text-gray-500">{t('suppliers.view.detailsSubtitle')}</p>
            </div>
          </div>
        ...
        <div className="p-6" id={`supplier-${supplier.id}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 text-xs text-gray-600">
            <div>
              <div>
                ...
                <span className="font-medium">{supplier.createdByName || '—'}</span>
              </div>
              <div>
                <span className="text-gray-500">{t('common.audit.createdAt')}:</span>{' '}
                <span className="font-medium">
                  {supplier.createdAt ? formatAppDateTime(supplier.createdAt, { locale: toLocale(lang) }) : '—'}
                </span>
              </div>
            </div>
            <div>
              <div>
                ...
                <span className="font-medium">{supplier.updatedByName || '—'}</span>
              </div>
              <div>
                <span className="text-gray-500">{t('common.audit.updatedAt')}:</span>{' '}
                <span className="font-medium">
                  {supplier.updatedAt ? formatAppDateTime(supplier.updatedAt, { locale: toLocale(lang) }) : '—'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('suppliers.view.basicInfoTitle')}</h3>
              <div className="space-y-4">
                <div className="flex items-center text-sm">
                  <User className="w-4 h-4 text-gray-400 mr-3" />
                  <div>
                    <span className="text-gray-600">{t('suppliers.modal.nameLabel')}:</span>
                ...
                <div className="flex items-center text-sm">
                  ...
                  <div>
                    <span className="text-gray-600">{t('suppliers.modal.emailLabel')}:</span>
                ...
                {!!supplier.phone && (
                  <div className="flex items-center text-sm">
                    ...
                    <div>
                      <span className="text-gray-600">{t('suppliers.modal.phoneLabel')}:</span>
                ...
                <div className="flex items-center text-sm">
                  ...
                  <div>
                    <span className="text-gray-600">{t('suppliers.view.registeredAt')}:</span>
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('suppliers.view.companyInfoTitle')}</h3>
              <div className="space-y-4">
                <div className="flex items-center text-sm">
                  <Building2 className="w-4 h-4 text-gray-400 mr-3" />
                  <div>
                    <span className="text-gray-600">{t('suppliers.modal.companyLabel')}:</span>
                    <span className="ml-2 font-medium text-gray-900">{supplier.company || '-'}</span>
                  </div>
                </div>
                <div className="flex items-center text-sm">
                  <div>
                    <span className="text-gray-600">{t('suppliers.modal.categoryLabel')}:</span>
                    <span className="ml-2">
                ...
                <div className="flex items-center text-sm">
                  <div>
                    <span className="text-gray-600">{t('suppliers.modal.taxNumberLabel')}:</span>
          ...
          {!!supplier.address && (
            <div className="mb-8">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('suppliers.view.addressInfoTitle')}</h3>
          ...
          <div className="bg-orange-50 rounded-lg p-4 border border-orange-200">
            <h3 className="text-lg font-semibold text-gray-900 mb-3">{t('suppliers.view.quickActionsTitle')}</h3>
            <div className="flex flex-wrap gap-3">
              {onCreateExpense && (
                ...
                >
                  <span>{t('suppliers.view.createExpense')}</span>
                </button>
              )}
              {onViewHistory && (
                <button
                ...
                >
                  <span>{t('suppliers.view.viewHistory')}</span>
                </button>
              )}
              <button
                onClick={() => {
